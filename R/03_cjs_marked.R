...
load(paste0('./data/cjs_model_data_sy',sy,'.Rda'))
...
df %>% 
  ...
  summarise(spawner_above = sum(spawner_above),
            ...
            kelt_LGR = sum(kelt_LGR),
            kelt_below = sum(kelt_below))


mod_dat <- data.frame(
  ch = paste0(df$spawner_above, df$kelt_LGR, df$kelt_below),
  freq = rep(1, dim(df)[1]),
  mpg = df$mpg,
  pop = df$popid,
  year = df$spawn_yr
...
save(df, mod_dat, kelt.cjs.models, best_mod, best_res, phi_lgd,  file =paste0('./data/model_data/mod_dat_sy', sy,'.Rda'))
