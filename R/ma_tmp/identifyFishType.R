...
steelhead_lifestage <- function(x, spawn_year = NULL, max_spawn_month = NULL){
  ...
  # find maximum spawner observation and movement
  max_obs <- x %>%
    filter(min_det < lubridate::ymd(paste0(spawn_year,'0701'))) %>% # before end of spawn year
    filter(direction != 'downstream') %>% #%in% c('start', upstream', 'no movement')) %>% # can't be moving downstream
  ...
  df <- x %>%
    ...
    mutate(life_stage_rk = case_when(
      min_det <= lubridate::ymd(paste0(spawn_year,'0801')) & node == 'BON' ~ 'kelt', 
...
# need to add a catch for spawners later than April that were observed previously someplace else
  ...
  df <- df %>%
    select(id, tag_code, life_stage, life_stage_rk,  node, direction, auto_keep_obs, user_keep_obs, min_det, everything())
...
steelheadCapHist <- function(x) {
 ...
 ch <- x %>%
    ...
    mutate(obs_loc = case_when(
      ...
      #str_starts(rkm, "522") & rkm_total > 695 & min_det > lgr_max_det & life_stage == 'spawner' ~ 'spawner_above',
      life_stage == 'spawner' & grepl('GRS', path) ~ 'spawner_below',
      life_stage == 'kelt' & !grepl('GRS', path) & node != 'LGR' ~ 'kelt_above',
      #str_starts(rkm, "522") & rkm_total > 695 & min_det > lgr_max_det & life_stage == 'kelt' ~ 'kelt_above',
      life_stage == 'kelt' & node %in% c('LGR', 'GRS') ~ 'kelt_LGR',
      ...
      #life_stage == 'kelt' & node %in% c('GOA', 'LMA', 'IHR', 'MCN', 'JDA', 'TDA', 'BON') ~ 'kelt_below',
      #life_stage == 'repeat spawner' & node != 'GRS' & grepl('GRS', path) ~ 'rs_below',
      TRUE ~ 'other')) %>%
    mutate(obs_loc = factor(obs_loc, levels = c('release_LGR', 'spawner_above', 'spawner_below', 'kelt_above', 'kelt_LGR', 'kelt_GOA', 'kelt_LMA', 'kelt_IHR', 'kelt_MCN', 'kelt_JDA', 'kelt_TDA', 'kelt_BON', 'rs_BON', 'rs_LGR', 'rs_above', 'other'))) %>%
    arrange(obs_loc) %>%
  ...
  col_order <- c('tag_code', 'release_LGR', 'spawner_above', 'spawner_below', 'kelt_above', 'kelt_LGR', 'kelt_GOA', 'kelt_LMA', 'kelt_IHR', 'kelt_MCN', 'kelt_JDA', 'kelt_TDA', 'kelt_BON', 'rs_BON', 'rs_LGR', 'rs_above', 'other')
