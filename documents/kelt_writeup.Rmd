...
load(paste0('../data/output/mod_dat_sy', sy, '_2025-04-03', '.rda')) # from 02_cjs_marked.R
ch_bio_df %<>%
...
We evaluated kelting rates of natural-origin adult steelhead observed spawning in Snake River tributaries upstream of Lower Granite Dam (LGR) moving back downstream past LGR, simultaneously estimating the kelt detection probability of LGR. The dataset used in this analysis comprises of observations of PIT-tagged natural-origin adult steelhead (newly or previously tagged) trapped at the LGR adult ladder during spawn years 2010 and subsequently detected near the spawning grounds at instream PIT-tag detection systems (IPTDS) or mark-recapture-recovery (MRR) sites. Post-spawn, PIT-tagged adults can additionally be "re-detected" moving downstream past LGR, at any downriver dam, or in the Columbia River estuary.
...
The best fitting CJS model included terms for spawn year, population, and sex for estimating apparent survival and only spawn year for estimating detection probabilities (Table \@ref(tab:mod-results)). Figures \@ref(fig:det-plot) and \@ref(fig:surv-plot) show the estimate detection and survival probabilities for the best fitting model. Table \@ref(tab:avg-surv-tab) and Figure \@ref(fig:avg-surv-plot) provide average survival estimates conditioned on population and sex and marginalized over spawn years [@casella_statistical_2021].
...
results %>%
  select(-time) %>%
  arrange(sex, -avg_Phi) %>%
  kable(col.names = c("TRT POPID", "Mean Survival", "Sex", "SE", "Lower 95% CI", "Upper 95% CI"),
        align = "cccccc",
        digits = 3,
        label = NA) %>%
  kable_styling(full_width = F,
