...
The dataset used in this analysis comprises of PIT-tag observations of adult steelhead (newly or previously tagged) trapped at the Lower Granite Dam (LGR) adult ladder during spawn years 2010 - 2024. We generated PIT-tag observations by first compiling a list of adult steelhead with *valid* PIT-tag codes collected at the LGR adult trap between July 1 and June 30 of each spawn year [@Baum2023; @Ogden2020]. All phenotypic natural-origin fish (e.g., present adipose fin and absence of a coded-wire tag) are included as part of the *valid* tag list. Any phenotypic natural-origin originating from hatchery broodstock is later identified using parentage-based tagging [@Steele2013; @Steele2019; @Baum2023]. For this analysis, we included natural-origin adults originating from hatchery broodstock (e.g., supplementation fish), but those could easily be removed or analyzed separately, if desired. A PIT-tag code was considered *valid* if the adult steelhead was trapped while ascending the LGR fish ladder during a systematic subsampling period, was greater than 30 cm FL, and not missing fields within the LGR trapping database [@Baum2023]. Once the valid tag was developed, we retrieved all PIT-tag interrogations for each code using the complete tag history query from [PTAGIS](https://www.ptagis.org). Observations were then processed within the [SnakeRiverFishStatus](https://github.com/NPTfisheries/SnakeRiverFishStatus) GitHub repo using scripts maintained there.

The full complete tag history was initially prcessed using the `PITcleanr::compression()` function [@See2024] in the R statistical software [@R-base]. This function, along with a supplied configuration file, collapsed multiple PIT-tag interrogations at the same location and within a similar time frame into a single record, retaining only essential information: the PIT-tag code, observation location (i.e., node), and the minimum and maximum timestamps of observations at that location (i.e., PIT-tag arrays or sites) into a single geographic location of interest. For instance, all PIT-tag detection sites at Bonneville Dam (e.g., BCC, B2J, etc.) were merged into a single node, while LGR sites were categorized into a downstream migration node (GRJ, GRS) and an upstream migration node (GRA, LGRLDR, LGR). All detections upstream of LGR remained distinct to facilitate assignment of detections to specific spawning locations and identify movement direction. Finally, we truncated the dataset to include only observations occurring after an upstream-migrating steelhead was first detected at the LGR adult trap, excluding all juvenile and pre-return adult observations.

The resulting dataset consists of PIT-tag observations from a randomly samples subset of adult steelhead migrating past LGR and potential subsequent detections indicating three distinct life stages: spawners, kelts, and repeat spawners. A simple rule set was applied to classify observations to each life stage using functions in the [SnakeRiverKelt](https://github.com/NPTfisheries/SnakeRiverKelt) GitHub repo. The spawning stage was assigned to all observations before the fish's last and furthest upstream movement. The kelt stage was assigned to observations occurring at least five days after the last upstream spawner movement in a downstream direction and to any detections at Columbia and lower Snake River hydrosystem sites after April 20. The repeat spawner stage was assigned to observations following the first upstream movement in any spawn year beyond the fish's initial spawn year.

In the `r n_distinct(df$spawn_yr)` years of observation data, we have observed `r prettyNum(dim(df)[1], big.mark = ",")` PIT-tagged adult steelhead at Snake River spawning locations. Tables \@ref(tab:data-year) and \@ref(tab:data-pop) show the number of these unique PIT-tagged spawners for each spawn year and within each Snake River population upstream of LGD. Additionally, the tables include the sum of unique kelt observations at LGR and observations of kelt downstream (i.e., below) of LGR.
